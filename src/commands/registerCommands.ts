...
| \`graph_link_entities\` | Create a typed relationship between entities. Use \`action: "unlink"\` to remove. |
| \`graph_delete_entity\` | Delete an entity. Provide \`observationId\` to delete only a specific observation. |
| \`graph_maintain\` | Maintenance: \`operation: "rebuild"\` / \`"compact"\` / \`"stats"\` |

### 6. Modes
| Tool | Purpose |
|------|---------|
| \`switch_mode\` | Switch to a mode: architect, code, ask, debug, or test. Call with no params to get current mode. Set \`umb:true\` to activate UMB, \`umb:false\` to deactivate. |
...
| Tool | Purpose |
|------|---------|
| \`create_backup\` | Create a timestamped backup. Use \`listOnly: true\` to list existing backups. |
...
| Tool | Purpose |
|------|---------|
| \`sequential_thinking\` | Record a numbered thinking step (raw thought NOT returned). Set \`reset: true\` to clear. |
